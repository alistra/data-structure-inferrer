...
\begin{abstract}
	...
	have libraries with convinience data structures implemented. Programmers just have to be taught when to
	...
	\missingfigure{ result, conclusion, http://www.ece.cmu.edu/~koopman/essays/abstract.html}
...
\section{Introduction} \label{sec:intro}
	...
	\subsection{What is dsinf?} \label{sub:intro}
		...
		possible) chosing the best matches and printing them out or linking them to your code to create a
		working binary.
		...
		The analysis process uses asymptotical complexity for data structure comparison. The process can also be
...
\section{Data structure inference}
	...
	\subsection{Collecting the program data} \label{dsu-definition}
		...
		We want to group the elements in $DSOPS(P)$ to detect the persistent identities \todo{ref okasaki} of
		data structures, meaning that in one group, there will be data structure operations conducted on one
		data structure from its allocation, to the end of the program, counting in passing the structure as a
		...
		\begin{itemize}
			...
			\item for every pair $(p, o_1)$ that was created by using the \autoref{it:param} from function
				...
				operations in one big group, just copy the operations to the according groups.

			\item for every \todo{copy propagation}
...
\section{Future work} \label{sec:future}
	It may be a good idea to rewrite the framework to analyze LLVM IR internally, instead of the C language AST. This
	could yield better results because of the optimizations that compiler can apply to the initial code, like
	removing unnecesary calls to data structure operations, so the framework can infer the data structure based on
	the optimized code. LLVM also enables a link-time optimization, so after linking the chosen data structure
	to a program, another optimization step is performed. Also analyzing LLVM IR is easier that analyzing
	the C language.

\begin{thebibliography}{9}
	\bibitem{Wi} http://en.wikipedia.org/
	\bibitem{AppleCC} https://developer.apple.com/library/mac/\#documentation/General/Conceptual/DevPedia-CocoaCore/ClassCluster.html
\end{thebibliography}
