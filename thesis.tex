	...
	\subsection{Choosing the best data structure} \label{sec:choose-ds}
		...
		\begin{table}[h!]
			...
			\begin{tabular}{|l|l|}
				...
				Update          & $O(lg \; n)$ \\
				Delete	        & $O(lg \; n)$ \\
				Find Max 	& $O(lg \; n)$/$O(1)$\footnote{Depending on implementation} \\
				Delete Max	& $O(lg \; n)$ \\
				Search 		& $O(lg \; n)$ \\
				Successor 	& $O(lg \; n)$/$O(1)$\footnote{Depending on implementation} \\
				\hline
			\end{tabular}
			\caption{A table showing the representation of Red-Black trees}
			\label{tab:rbt}
		\end{table}

		\todo{footnote in tabular}
		...
		\todo{not implementing all the ops - what then?}
