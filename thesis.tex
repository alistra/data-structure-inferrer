	...
	\subsection{Importance of operations} \label{sub:importance}
		...
		\subsubsection{Choosing the best data structure with weights} \label{sec:choose-weights}
            ...
            \begin{equation} \label{data-structure-order-weights}
                d_1 <_{ds} d_2 \Leftrightarrow OPCOST(d_1, ds) <= OPCOST(d_2, ds)
            \end{equation}

            where:

            \begin{equation}
                DSCOST(d,ds) = \sum \{OPCOST(o,c,ds) | (o,c) in \d, o \in ds\}
            \end{equation}
            \begin{equation} \label{data-structure-order-weights}
                OPCOST(o,c,ds) = c(N) * w(o, ds) \texttt{ where } w(o, ds) is the weight of the operation o in ds
            \end{equation}
