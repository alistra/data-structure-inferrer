...
\section{Extensions of the idea}
	...
	\subsection{Different element types}
		...
		\subsubsection{Linked data structures}
            ...
            the given order; an example of a general operation is \emph{insert\_d}, which doesn't take the order as an
            argument, it has to perform an operation on both linked data structures. 
			...
			\begin{eqnarray}
                RELATED :: DataStructureOperation -> Maybe DataStructureOperation
            \end{eqnarray}

            For read-only general operations the function is defined as:

			\begin{eqnarray}
                RELATED(Size) = Minimal cost of on any structure \\
                RELATED(AnyElement) = Minimal cost of on any structure
            \end{eqnarray}
			...
			\begin{eqnarray}
                RELATED(Insert) = Just Insert\\
                RELATED(Delete) = Just Delete
            \end{eqnarray}

            For read-only order specific operations he function is defined as:

			\begin{eqnarray}
                RELATED(FindMax) = Nothing\\
                RELATED(FindMin) = Nothing\\
                RELATED(Find) = Nothing\\
                RELATED(Successor) = Nothing\\
                RELATED(Predecessor) = Nothing\\
            \end{eqnarray}

			For read-write order specific operations he function is defined as:

			\begin{eqnarray}
                RELATED(DeleteMax) = Just Delete\\
                RELATED(DeleteMin) = Just Delete
            \end{eqnarray}
            ...
            First we modify the $DSU()$ definition for record variables.
           	...
           	NOT ALWAYS
            For the final cost of an operation on a linked data structure we take the sum of costs of operations on all the data structures, which is big-O-wise just the most costly operation.

            \begin{equation} \label{data-structure-order-different-types}
                ld_1 <_{dsu} ld_2 \Leftrightarrow DSCOST(ld_1, dsu) \leq DSCOST(ld_2, dsu) 
            \end{equation}
            ...
            \begin{eqnarray}
                DSCOST(ld,dsu) = \sum_{u \in dsu} OPCOST(u, ld)
            \end{eqnarray}

			\emph{OPCOST} describes the cost of a single usage \emph{u} on the linked data structure \emph{ld}.

            \begin{equation}
                OPCOST(u, ld) = \begin{cases}
   						\min_{d \in ld} SINGLEOPCOST(u,d) & \text{if } u = (v, o, w) \wedge o \text{ read-only order independent operation} \\
   						\sum_{d \in ld} SINGLEOPCOST(u,d) + RELCOST(u, d, ld) & \mathit{otherwise}
  					\end{cases}
            \end{equation}

            \emph{SINGLEOPCOST} describes the cost of a single usage \emph{u} on one of the substructures \emph{d}
            related to the ordering the operation uses.

			\begin{eqnarray}
				SINGLEOPCOST(u,d) = c(N) * w \; \textrm{where} \; (o,c) \in d, u = (v, o, w)
            \end{eqnarray}

			\emph{RELCOST} describes the cost of a single usage \emph{u} on all the substructures other than \emph{d}.

            \begin{eqnarray}
                RELCOST(u, d, ld) = \sum_{d' \in ld, d \neq d'} SINGLEOPCOST(u,d') HERE ADD OTHER COST WITH RELATED()
            \end{eqnarray}


            
            \missingfigure{Finish the rest}
